...
/*   Updated: 2023/03/21 19:15:23 by seokang          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void ft_set_check_map(t_game *game, char mode)
{
    ...
    game->check.check = (int *)malloc(sizeof(int) * ft_strlen(game->map.map_str));
	if (!game->check.check)
...
static void ft_floodfill(int col, int row, t_game *game)
{
    ...
    if (col >= game->map.col || row >= game->map.row || col < 0 || row < 0 || game->check.check[idx] != 0)
        return ;
